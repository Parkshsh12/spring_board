...
public class CommonExceptionHandler {
	...
	// protected ModelAndView handleInternalServerError(HttpServletRequest req,
	// Exception e) {
	// logger.error("Exception: {}"+ e.toString());
	// printExceptionInfo(req, e);
	//
	// ModelAndView mv = new
	// ModelAndView(BoardUINameEnum.JSP_EXCEPTION.getJSPFileName());
	...
	// protected ModelAndView runtimeException(HttpServletRequest req,
	 ...
	 * @메소드 설명: RuntimeException 발생시 공통 Exception 처리 1. LOG 남기기. 2. DB Exception
	 *      Table에 추가. 3. Message or ModelAndView return.
	 *      □■□■□■□■□■□■□■□■□■□■□■□■□■□■□■□■□■□■□■□■□■□■□■□■□■
	 */
	@ExceptionHandler(RuntimeException.class)
	@ResponseBody
